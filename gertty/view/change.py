        ...
        with self.app.db.getSession() as session:
            ...
            for message in change.messages:
                if (message.revision == change.revisions[-1] and
                    ...
                    commentlink = next(
                        (cl for cl in self.app.config.commentlinks
                         if cl.test_result), None)

                    if commentlink:
                        results = commentlink.getTestResults(
                            self.app, message.message)
                        if results:
                            system_commentlink = next(
                                (cl for cl in self.app.config.commentlinks
                                 if cl.test_result_system), None)
                            if system_commentlink:
                                system = system_commentlink.getTestResultSystem(
                                    self.app, message)
                            else:
                            ...
                            if system in result_systems:
                                if system != message.author.name:
                                    # We were able to detect a pipeline, this
                                    # means we can accurately count rechecks
                                    result_system['rechecks'] += 1
                                if message.created > result_system['datetime']:
                                    result_system['results'].update(results)
